...
class EGCointegration(Strategy):
    ...
    def __init__(self, x, y, on, col_name, is_cleaned=True):
        ...
        self.MTR_variables = False
        self.p_values_MTR = []
        self.orders_MTR = []
        self.spread_MTR = []
        self.hist_pri_diff = (y['close'] - x['close']).mean()
    ...
    def cal_spread(self, x, y, is_norm):
        resid = y - x * self.beta
        resid = (resid - resid.mean()) / resid.std() if is_norm is True else resid # MTR
        #resid = (resid - self.resid_mean) / self.resid_std if is_norm is True else resid # Latest version
        return resid
    ...
    def calibrate(self, start, end, cl):
        ...
        if self.p < cl:
            self.run_ols(x, y)
            #resid = self.cal_spread(x, y, is_norm=False) # MTR
            #self.resid_mean = resid.mean() # MTR
            #self.resid_std  = resid.std() # MTR
            # If i want to calculate the historical spread, i can make a call to cal_spread here.

    def gen_signal(self, start, end, trade_th, stop_loss, transaction_cost):
    # self.gen_signal(start_forward, end_forward, trade_th, stop_loss, transaction_cost)
        stop_loss  = trade_th + stop_loss
        x, y, time = self.get_sample(start, end)
        spread = self.cal_spread(x, y, is_norm=True)
        price  = self.cal_spread(x, y, is_norm=False)

        if self.MTR_variables:
            self.spread_MTR.append(spread) # MTR
        ...
        # ind_buy/ind_sell: will be True at indexes at which a buy/sell (long/short) of spread is to be performed, otherwise False 
        ind_buy  = np.logical_and(spread_t0 > -trade_th, spread_t1 <= -trade_th).reshape(-1, )
        # spread_t0[i+1]<= -trade_th < spread_t0[i] : spread just passed the negative trading threshold from above

        ind_sell = np.logical_and(spread_t0 <  trade_th, spread_t1 >=  trade_th).reshape(-1, )
        # spread_t0[i] < trade_th <=spread_t0[i+1] : spread just passed the positive trading threshold from below
        ...
        # ind_stop
        # Four logical_and s: 
        # (spread_t0[i+1]   <= -stop_loss   <   spread_t0[i])           # spread just passed the negative stop threshold
        # (spread_t0[i]     < stop_loss     <=  spread_t0[i+1])         # spread just passed the positive stop threshold
        # (spread_t0[i+1]   <= 0            <   spread_t0[i])           # spread just passed the mean from above
        # (spread_t0[i]     < 0             <=  spread_t0[i+1])         # spread just passed the mean from below
        # logical_or s: stop if any of the above conditions are met.
        # 
        # 
        ################################################
    ...
    def process(self, n_hist, n_forward, trade_th, stop_loss, cl, transaction_cost, index=None, **kwargs):
        ...
        self.record = None
        if self.MTR_variables:
            self.p_values_MTR.append(self.p)
        if self.p < cl:
            time, price, order, gross_exp, cost = self.gen_signal(start_forward, end_forward, trade_th, stop_loss, transaction_cost)
            # The first index of trade_record will be start_forward + 1 because the time that gen_signal returns is t_t1[ind_order].
            # and t_t1 = time[1:]
            trade_record = self.gen_trade_record(time, price, order, cost)
            returns      = (trade_record['profit'] -
                            trade_record['trade_cost'] -
